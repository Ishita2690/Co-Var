...
if [ $# -ne 7 ]
then
  echo "Usage: $0 <CoVarS> <Zscores> <SeqLen1> <SeqLen2> <Dir> <nam1> <nam2>" 1>&2
  exit 1;
fi
...
image2D( CB, xv, yv , xlab = \"Residue No. (Reference Sequence $6)\", ylab = \"Residue No. (Reference Sequence $7)\" , main = \"Co-variation Scores for co-evolving positions\");

dev.off();

png(\"$5/SeqZplot.png\", width = 6, height = 6, res= 600, units = 'in');
image2D( ZB, xv, yv , xlab = \"Residue No. (Reference Sequence $6)\", ylab = \"Residue No. (Reference Sequence $7)\" , main = \"Z-scores for co-evolving positions\");
